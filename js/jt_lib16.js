function JT(id,w,h,fps,setupName,updateName,objName,mobileAudioSize,fullScreenBtn){
	...
	this.particles={
		draw:undefined,
		parts:[],
		addParticle:function(x,y,w,h,frames,wRate,hRate,alpha,alphaRate,vX,vY,aX,aY,c,cRate,cMax,r,rRate,id){
			var part={};
			part.x=x;
		...
		updateParticle:function(i){
			...
			if(p.frame>=p.frames || p.alpha<=0 || p.w<=0 || (p.h<=0 && p.hRate!=0)){dead=true;}
			p.frame++;
			return dead;
		},
		...
		drawParticle:function(i){
			...
			if(p.h==0 && p.hRate==0){
				this.draw.circle(p.x-p.w/2,p.y-p.h/2,p.w,p.c);
			}else{
	...
	this.addPart=function(x,y,w,h,frames,wRate,hRate,alpha,alphaRate,vX,vY,aX,aY,c,cRate,cMax,r,rRate,id){
		if(typeof x=="object"){
